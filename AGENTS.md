...
**Residual risk, stated not silent**: the Slack tool-name confirmation above came from `prompts/system.md`'s documented names, not a live introspection of Rachel's own Slack MCP connector (this session's attached Slack connector wasn't the same authenticated instance). If Rachel's live tool names ever drift from what's documented here, `GATED_TOOL_NAMES` in `gate/sendGate.ts` silently stops matching and the gate stops covering the real send path. Closing this needs either a startup-time live schema check or a future hook-probe run confirming tool names directly inside a Rachel session — not yet built.

**Fail-closed semantics, confirmed by live spikes against the installed SDK** (Node v24.10.0, `@anthropic-ai/claude-agent-sdk`). The two failure modes now behave differently, and the split is version-dependent:

- **A hook that throws is swallowed and treated as no-opinion — the tool proceeds. Confirmed fail-open, unchanged.** Re-verified on 0.3.216 (2026-07-21): a `PreToolUse` hook throwing on every tool call did not stop the write.
- **A hook resolving after its declared `HookCallbackMatcher.timeout` now fails CLOSED.** On the 0.2-era SDK this was fail-open — the SDK did not cut the hook off and the tool proceeded. Re-verified on 0.3.216 (2026-07-21) with a 1s declared timeout against an 8s hook: the tool call was blocked, not allowed. `timeout` is now a real enforcement mechanism; it was not before.

Consequence, unchanged by the above: `createSendGateHook` wraps its entire body in try/catch (explicit deny on any exception) and races every approval surface against its own internal deny-timer (`Promise.race`, strictly shorter than any matcher timeout) rather than trusting the SDK to fail closed on its own. The try/catch is still load-bearing — the throw path remains fail-open. The internal deny-timer is now defence-in-depth rather than the sole timeout enforcement, and being strictly shorter than the matcher timeout means it still settles first. Do not relax either without re-running both spikes: the throw/timeout split changed once across a minor version and can change again.

**Per-item approval, one-shot, audited**: approval is bound to a SHA-256 hash of the canonicalised (sorted-key) tool input — approving one message never approves a different one, even to the same tool. An approval is consumed on first use; a replay of the same hash is denied (`"Approval already consumed — request a fresh approval."`). Every attempt and decision is appended to `~/.rachel/send-gate-audit.jsonl`. A well-formed send with no approval received is denied with `"No approval received — redraft or ask the operator directly."` A `Bash` command matching a known send-API pattern (Slack `chat.postMessage`, Telegram `sendMessage`, Gmail `messages/send`, or a `POST` to the Calendar events endpoint) is denied outright with `"Send-capable Bash command blocked — use the corresponding MCP tool instead (Slack/Gmail/Calendar), which routes through the approval gate."` — this path has no approval to offer, since a Bash send bypasses hash-binding entirely.
