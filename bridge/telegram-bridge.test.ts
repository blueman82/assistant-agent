...
test("ticker: a render already in flight when the turn ends must not re-arm — no working edit is ISSUED after the terminal edit", async () => {
  // Record issue time (when the fetch call starts, before any artificial
  // delay) for every editMessageText call, not merely the order calls
  // resolve/record in. A render already in flight at turn-end is expected
  // to still complete its already-issued call — the fix does not (and
  // cannot) un-send it — so a record-order assertion would be satisfied
  // trivially by that one legitimate straggler and couldn't tell "one
  // expected straggler" apart from "the guard is disabled and it keeps
  // re-arming". The actual contract under test is narrower: once the turn
  // ends, no NEW working edit may be ISSUED — proven by comparing each
  // edit's issue timestamp against the terminal edit's issue timestamp,
  // not against when calls happened to resolve.
  const { transport } = makeStubTransport([
    messageUpdate(1, "run a fast job"),
    { ok: true, result: [] },
  ]);
  const issuedEdits: { text: string; issuedAtMs: number }[] = [];
  let firstEditDelayed = false;
  const delayedTransport: typeof transport = async (input, init) => {
    const url = String(input);
    if (url.includes("/sendMessage")) return { ok: true, json: async () => ({ ok: true, result: { message_id: 9001 } }) } as Response;
    if (url.includes("/editMessageText")) {
      ...
      issuedEdits.push({ text, issuedAtMs });
      // Only the FIRST render (the grace-timer's initial "working 0m00s",
      // armed while the turn is still running) is delayed past turn-end —
      // it is the one call genuinely in flight when the turn's finally
      // block runs. Every later call (including the terminal edit, and any
      // stale re-arm if the guard is disabled) resolves immediately, so
      // its issue time is what the assertion below actually depends on.
      if (!firstEditDelayed) {
  ...
  // Long enough that the grace-timer's first render is issued (and parked
  // in the delayed editMessageText above) while the turn is still running,
  // but short enough that the turn ends well before that render's 300ms
  // delay resolves — the parked callback is genuinely in flight at
  // turn-end, not merely scheduled.
  const runTurnStub: BridgeRunTurn = async (_input, emit) => {
    emit("  [Bash] fast-step", "tool");
    await new Promise((r) => setTimeout(r, 50));
    emit("Fast job done.", "text");
  };

  const bridge = createBridge({
    ...basePushOpts(),
    config: { token: "t", chatId: "12345", transport: delayedTransport },
    runTurn: runTurnStub,
    getSessionId: () => undefined,
    resetSession: () => {},
    pollIntervalMs: 5,
    typingIntervalMs: 100000,
    tickerGraceMs: 5,
    tickerJitterMinMs: 10,
    tickerJitterMaxMs: 10,
  });

  await bridge.drainOnce();
  // The turn (≈50ms) finishes well before the delayed first edit (300ms)
  // resolves. Wait well past that delay — and past the 1s mark where the
  // rendered ticker text changes from "working 0m00s" to "working 0m01s",
  // so a re-armed render (guard disabled) would produce a NEW, distinct
  // edit rather than being skipped by the unchanged-text short-circuit —
  // then stop the bridge and inspect what actually landed.
  await new Promise((resolve) => setTimeout(resolve, 1300));
  await bridge.stop();

  const terminal = issuedEdits.find((e) => e.text.startsWith("done —"));
  assert.ok(terminal, `expected a terminal 'done —' edit, got: ${JSON.stringify(issuedEdits)}`);
  const workingAfterTerminal = issuedEdits.filter(
    (e) => e.text.startsWith("working") && e.issuedAtMs > terminal!.issuedAtMs,
  );
  assert.equal(
    workingAfterTerminal.length,
    0,
    `no 'working…' edit may be ISSUED after the terminal edit is issued, got: ${JSON.stringify(issuedEdits)}`,
  );
});
