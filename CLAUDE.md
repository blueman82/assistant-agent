...
To understand the agent you must read both files together — `rachel.ts` tells you which tools are wired in; `system.md` tells you how they're meant to be used. `bridge/telegram-bridge.ts` is a thin transport layer on top of both — it adds no new behaviour, only a second way to reach the same `runTurn`.
